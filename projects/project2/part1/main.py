...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    print(f'softmax test_error_mod3 (temp={temp_parameter}) = {test_error_mod3}')
    return test_error
...
print(f'softmax test_error com PCA 18 componentes (temp=1) = {test_error}')
...
print(f'Error rate for 10-dimensional cubic PCA features = {test_error_cube}')

# Polynomial svm using scikit-learn
from sklearn.svm import SVC
poly_svm = SVC(random_state=0, kernel='poly', degree=3)
poly_svm.fit(train_pca10, train_y)
pred_test_y_poly = poly_svm.predict(test_pca10)
test_error_poly = compute_test_error_svm(test_y, pred_test_y_poly)
print(f'Error rate for 10-dimensional PCA features using cubic polynomial svm = {test_error_poly}')

# Rbf svm using scikit-learn
rbf_svm = SVC(random_state=0, kernel='rbf')
rbf_svm.fit(train_pca10, train_y)
pred_test_y_rbf = rbf_svm.predict(test_pca10)
test_error_rbf = compute_test_error_svm(test_y, pred_test_y_rbf)
print(f'Error rate for 10-dimensional PCA features using rbf svm = {test_error_rbf}')